...
import { getStocksList, getMoreStocks, searchStock, addStock } from './stocksSlice';
...
function Stocks() {
  ...
  const convert = async (from, to) => {
    ...
    for(let stock of favouriteStocks) {
      const obj = {};
      const convertedPrice = await axios.get(`https://api.exchangerate.host/convert?from=${from}&to=${to}&amount=${stock.price}`);
  ...
  const addStockToFavourites = (stock) => {
    ...
    if(isFound) {
      ...
      const currentList = JSON.parse(localStorage.getItem("favouriteStocksList"));
  ...
  const removeStockFromFavourites = (symbol) => {
    const favouriteStocksList = JSON.parse(localStorage.getItem("favouriteStocksList"));
    const modifiedFavouriteStocksList = favouriteStocksList.filter((stock, index) => stock.symbol !== symbol);
    localStorage.setItem("favouriteStocksList", JSON.stringify(modifiedFavouriteStocksList));
  ...
  return (
    <div className="Stocks">
              ...
              <div className="stocks" key="uniqueDiv">
                {
                  favouriteStocks.map((stock, index) => (
                        <Card id={`favouriteStock-${stock.symbol}`} style={{ width: '18rem', margin: '0 auto', textAlign: 'center' }} key={stock.symbol} >
                          <Card.Body>
                            <Link to={`/stocks/${stock.symbol}`}>
                              <Card.Title className="stockTitle">{stock.symbol}</Card.Title>
                            </Link>
                            ...
                            <div>
                              <Button variant="outline-danger" onClick={() => removeStockFromFavourites(stock.symbol)}>Delete from favourites</Button>
                            </div>
                          </Card.Body>
                        </Card>
                  )) 
      ...
      <div className="stocksInput" style={{padding: favouriteStocks.length === 0 ? '8% 2% 4% 2%' : '4%'}}> 
        <Form.Control
          type="text"
            ...
            >
              {
                tempStocksList.length === 0 ?
                  ...
                  tempStocksList.map((stock, index) => (
                    <Card style={{ width: '18rem', margin: '0 auto', textAlign: 'center' }} key={index}>
                      <Card.Body>
                        <Link to={`/stocks/${stock.symbol}`}>
                          <Card.Title className="stockTitle">{stock.symbol}</Card.Title>
                        </Link>
                        ...
                        <hr />
                        <Card.Text>
                          Share price {stock.price} USD
                        </Card.Text>
                        <Card.Text>
                          {stock.exchange} - {stock.exchangeShortName}
                        </Card.Text>
                        <div>
                          <Button variant="outline-secondary" onClick={() => addStockToFavourites(stock)}>Add to favourites</Button>
                        </div>
                      </Card.Body>
                  </Card>
                  ))
